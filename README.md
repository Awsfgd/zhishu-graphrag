...
**一个企业级的「多Agent协作」知识管理系�?*

4个AI Agent分工协作，完成企业知识的全生命周期管理：文档解析 �?知识抽取 �?智能问答 �?增量更新

[快速开始](#-快速开�? · [系统架构](#-系统架构) · [功能演示](#-功能演示) · [API文档](#-api-接口) · [面试资料](#-面试资料)
...
### 什么是 Agent�?
Agent（智能体）就是一�?能思考、能执行"的AI程序。它可以�?
- 理解你的需求（自然语言�?
...
**ZhiShu** 包含 **4个核心Agent**，通过 [LangGraph](https://langchain-ai.github.io/langgraph/) 有向图编排，实现企业知识的全链路智能处理�?

### 4个Agent是什么，分别做什么？

| Agent | 中文�?| 职责 | 类比理解 |
|-------|--------|------|----------|
| `DocParserAgent` | 文档解析Agent | 把PDF/图片/表格等各种格式的文档"读懂"，切割成小段�?| 超强秘书，能看懂任何格式的文�?|
| `KnowledgeExtractAgent` | 知识抽取Agent | 从文本中自动提取人名、公司、关系等结构化信�?| 分析师，把信息整理成知识图谱 |
| `QAAgent` | 问答Agent | 接收用户问题，同时查向量库和知识图谱，生成精准答�?| 专家顾问，综合多源信息回�?|
| `KnowledgeUpdateAgent` | 知识更新Agent | 监听文档变更，只更新变化的部分，保持知识库最�?| 勤快管理员，实时维护知识�?|

### 三大技术亮�?

| 亮点 | 说明 | 解决什么问�?|
|------|------|-------------|
| **多模态RAG** | 不只处理文字，还能理解PDF里的图片、表格、流程图 | 传统系统只能处理纯文�?|
| **GraphRAG (知识图谱)** | 用图数据库存储实体关系，支持多跳推理 | 纯向量检索无法处�?关系�?�?多步推理"问题 |
| **CDC增量更新** | 文档变了只更新变化的部分 | 传统方案每次全量重建�?000个文档改5个要30分钟 |
...
| **向量数据�?* | [ChromaDB](https://www.trychroma.com/) / [PGVector](https://github.com/pgvector/pgvector) | ChromaDB开箱即用；PGVector适合已有PostgreSQL的企�?|
...
| **容器�?* | [Docker Compose](https://docs.docker.com/compose/) | 一键启动所有依赖服�?|
...
| 组件 | 技术选型 | 说明 |
|------|----------|------|
| **API框架** | [Gin](https://gin-gonic.com/) | Go生态最流行的HTTP框架，极低延�?|
...
| **向量存储** | [pgvector-go](https://github.com/pgvector/pgvector-go) | PostgreSQL向量扩展Go客户�?|
| **并发** | goroutine | Go原生并发，文档批量解析天然并�?|

---

## 🌐 三语言实现

本项目同时提�?Python、Java、Go 三种语言实现�?*架构设计完全一�?*，只是语言和框架不同�?

| 语言 | 适合什么岗�?| 特点 | 目录 |
|------|------------|------|------|
| **Python** | AI工程师、算法工程师、数据工程师 | 功能最完整，LangGraph原生支持，推荐学习入�?| [`python/`](./python/) |
...
- 一�?OpenAI API Key（或者用国内兼容接口，见下方FAQ�?

### 步骤1：克隆项�?

```bash
git clone https://github.com/bcefghj/zhishu-graphrag.git
cd zhishu-graphrag
```
...
打开浏览器访�?[http://localhost:8080/docs](http://localhost:8080/docs)，可以看到交互式API文档�?
...
chunks = await agent.parse("财务数据.xlsx")   # Excel �?结构化文�?
chunks = await agent.parse("产品文档.md")     # Markdown �?纯文�?
...
print(result.sources)    # 来源引用（哪些文�?哪些知识图谱节点�?
print(result.confidence) # 置信度分�?

# 内部执行流程�?
# 1. 向量检�?�?找到语义相关的文档段落（用余弦相似度�?
# 2. 实体链接 �?识别问题中的"张三"是哪个实�?
# 3. 图谱检�?�?张三 �?负责 �?微信 �?竞争对手 �?QQ / 钉钉
# 4. 混合重排�?�?图谱路径结果权重×1.25（推理链更精准）
# 5. LLM生成 �?综合所有信息，生成结构化答�?
```
...
�?
├── README.md                          �?你正在看的这个文�?
├── docker-compose.yml                 �?一键启动所有依赖服�?
�?
├── docs/                              �?文档目录
�?  ├── architecture.md                �?架构设计详解（每个决策的理由�?
�?  ├── interview-guide.md             �?面试八股�?+ STAR法则话术
�?  ├── resume-template.md             �?简历写法模�?
...
�?  ├── orchestrator/
�?  �?  └── graph.py                   �?LangGraph编排引擎（定�?条流水线�?
�?  ├── services/
�?  �?  ├── vector_store.py            �?向量库服务（ChromaDB/PGVector�?
�?  �?  ├── knowledge_graph.py         �?知识图谱服务（Neo4j�?
�?  �?  ├── graph_rag.py               �?GraphRAG混合检索管�?
�?  �?  ├── cdc_processor.py           �?CDC增量更新处理�?
�?  �?  └── multimodal.py              �?多模态处理服�?
�?  ├── api/
...
启动服务后，访问 [http://localhost:8080/docs](http://localhost:8080/docs) 查看交互�?Swagger API 文档�?
...
| `POST` | `/api/ingest/batch` | 批量上传文档 | 上传多个文件，自动并行处�?|
...
```json
{
  ...
  "sources": [
    ...
    {"entity": "张三", "relation": "就职�?, "target": "腾讯", "type": "graph"}
  ]
}
```
...
| 文档 | 内容 | 什么时候用 |
|------|------|----------|
| [**架构设计详解**](./docs/architecture.md) | 每个技术决策的理由（为什么用LangGraph？为什么用GraphRAG？） | 面试被深追问�?|
| [**面试八股�?STAR**](./docs/interview-guide.md) | 30+高频面试�?+ STAR话术模板 | 面试�?天突�?|
| [**简历写法模�?*](./docs/resume-template.md) | 怎么把这个项目写进简历（量化指标怎么写） | 投简历前 |
| [**核心代码讲解**](./docs/tech-deep-dive.md) | 关键代码逐行解读，搞懂原�?| 代码层面被追问时 |
| [**项目规划方案**](./docs/project-plan.md) | 完整的项目设计方�?| 理解整体思路 |

### 面试中如何介绍这个项目（STAR法则�?

**S（背景）**: 企业内部文档知识管理效率低下，传统关键词搜索准确率只�?0%，无法处理多格式文档和多跳推理问题�?

**T（任务）**: 设计并实现一个多Agent协作的企业知识管理系统，支持多模态文档处理和智能问答�?

**A（行动）**: 
- 设计�?个专职Agent的分工协作架�?
- 引入GraphRAG融合向量检索和知识图谱检�?
- 实现了CDC增量更新机制，避免全量重建的性能损�?
- 使用LangGraph有向图编�?条工作流水线

**R（结果）**: 
- 问答准确率从60%提升�?4%
- 文档更新响应时间�?0分钟缩短�?0�?
- 支持PDF/图片/Excel/Markdown等多种格�?
...
这是一�?*架构展示 + 学习项目**，展示了企业级系统的设计思路。如果要用在生产环境，还需要补充：
...
**三个版本架构设计完全一�?*，只是实现语言不同�?

- **Python�?*：功能最完整，推荐作为学习入口。面AI工程�?算法工程师岗位必备�?
- **Java�?*：用Spring生态实现，适合面Java后端/架构师岗位�?
- **Go�?*：适合面基础架构/云原�?高性能后端岗位�?
...
- [ChromaDB 官方文档](https://docs.trychroma.com/) �?向量数据�?
...
- 发现 Bug？[提交 Issue](https://github.com/bcefghj/zhishu-graphrag/issues)
- 想加新功能？欢迎 Fork 后提 PR
- 觉得有帮助？请点�?�?Star，这是对我最大的鼓励�?
